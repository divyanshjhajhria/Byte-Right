...
function generateFromLocal(PDO $db, float $budget, array $dietaryPrefs, string $timePref): array {
    ...
    if (empty($recipes)) {
        // If too restrictive, get all recipes
    ...
    $breakfastRecipes = array_filter($recipes, fn($r) => str_contains($r['tags'] ?? '', 'breakfast'));
    $dinnerRecipes = array_filter($recipes, fn($r) =>
    ...
    $breakfastRecipes = array_values($breakfastRecipes);
    $dinnerRecipes = array_values($dinnerRecipes);

    // Sort by cost so we can pick budget-appropriate meals
    usort($breakfastRecipes, fn($a, $b) => ($a['estimated_cost'] ?? 0) <=> ($b['estimated_cost'] ?? 0));
    usort($dinnerRecipes, fn($a, $b) => ($a['estimated_cost'] ?? 0) <=> ($b['estimated_cost'] ?? 0));

    $dailyBudget = $budget / 7;

    // Split recipes into tiers based on daily budget
    // Breakfast gets ~30% of daily budget, dinner gets ~70%
    $breakfastBudget = $dailyBudget * 0.30;
    $dinnerBudget = $dailyBudget * 0.70;

    $affordableBreakfasts = array_values(array_filter($breakfastRecipes, fn($r) =>
        ($r['estimated_cost'] ?? 0) <= $breakfastBudget * 1.2  // 20% wiggle room
    ));
    $affordableDinners = array_values(array_filter($dinnerRecipes, fn($r) =>
        ($r['estimated_cost'] ?? 0) <= $dinnerBudget * 1.2
    ));

    // Fall back to all if budget is too tight for any matches
    if (empty($affordableBreakfasts)) $affordableBreakfasts = $breakfastRecipes;
    if (empty($affordableDinners)) $affordableDinners = $dinnerRecipes;

    // Shuffle within the affordable pool for variety
    shuffle($affordableBreakfasts);
    shuffle($affordableDinners);

    $plan = [];
    $totalCost = 0;

    for ($day = 0; $day < 7; $day++) {
        $bRecipe = $affordableBreakfasts[$day % count($affordableBreakfasts)];
        $dRecipe = $affordableDinners[$day % count($affordableDinners)];

        $dayCost = ($bRecipe['estimated_cost'] ?? 0) + ($dRecipe['estimated_cost'] ?? 0);

        // If this day blows the daily budget, find a cheaper dinner
        if ($dayCost > $dailyBudget && count($affordableDinners) > 1) {
            foreach ($affordableDinners as $alt) {
                $altCost = ($bRecipe['estimated_cost'] ?? 0) + ($alt['estimated_cost'] ?? 0);
                if ($altCost <= $dailyBudget) {
                    $dRecipe = $alt;
                    $dayCost = $altCost;
                    break;
                }
